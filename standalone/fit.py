...
#  Pair range, in units of the lattice constant.  Overridable so the cutoff can
#  be varied against the taper: a smooth switch also shortens the effective
#  range, and telling those two apart needs the hard truncation moved to where
#  the switch takes over.
RCUT2_OVER_A = float(os.environ.get("RCUT2_OVER_A", "2.6"))

#  Three-body range, in units of the nearest-neighbour distance.  1.12 keeps the
#  first shell only, which is a modelling choice made here and NOT what the
#  original work did: G. Ugur's thesis (section 3.3) states that both the
#  two-body and the three-body sums were carried to the tenth neighbour shell.
#  For niobium that is 8.58 A - which is, to three figures, the RCUT2_OVER_A
#  above, so the pair range already matches; only phi3 is truncated short.  The
#  cost of the difference is not small: 28 triplets per atom here against 9180
#  at ten shells, 66 against 19900 for copper.  Overridable so the effect can be
#  measured rather than assumed.
#  1.50 lands on the second neighbour shell for fcc and bcc and the third for
#  hcp, which a shell-by-shell scan on nine elements found to be the optimum in
#  every structure.  Measured against the old first-shell value (20-40 restarts,
#  matched): Nb 216.7 -> 21.0, Ta 48.3 -> 11.5, W 49.9 -> 32.6, Al 22.0 -> 7.0,
#  Pt 9.4 -> 3.4, Ti 12.4 -> 5.6, Zr 11.3 -> 8.3, Cu 3.7 -> 0.0.  Magnesium is
#  the one regression, 13.1 -> 14.8; it prefers the fourth shell.
#
#  Longer is NOT better, and that was checked rather than assumed: aluminium
#  reaches 7.0 % here and 12.1 % at the third shell, platinum 3.4 % against
#  6.1 %, with the restart count doubled to rule out the search being the cause.
RCUT3_OVER_DNN = float(os.environ.get("RCUT3_OVER_DNN", "1.50"))

#  Cutoff taper, as a fraction of each cutoff; see latdyn.Potential.
#
#  Hard truncation is consistent at a fixed geometry and is what the published
#  library was fitted with, so this defaults to off and those fits reproduce
#  exactly.  It has to be on for anything that moves: phi2 does not vanish at
#  rcut2, so a neighbour crossing the sphere changes the energy in one step -
#  0.16 eV/atom for beryllium compressed by under one per cent.
#
#  Turning it on invalidates the parameters rather than perturbing them; the
#  tail carries real binding, and re-evaluating the shipped fits with the taper
#  moves the bulk modulus by hundreds of per cent.  It is a refit, not a
#  correction, which is why it is a switch and not a constant.
TAPER = (float(os.environ["TAPER"]) if os.environ.get("TAPER") else None)

R0_MIN_FRAC, R0_MAX_FRAC = 0.70, 1.50
D_MIN_FRAC = 0.05
C_MAX = 60.0
#  The three-body term may correct the pair term; it may not cancel it.  3.0
#  was effectively no constraint at all: chromium, molybdenum and tungsten came
#  through the tapered refit at 0.55-0.62, meaning phi2 supplied -10 to -20
#  eV/atom and phi3 gave +6 to +11 back to leave a cohesive energy near 4-9.
#  Two large numbers cancelling to a small one is not a description of a metal,
...
#  makes the first shell repulsive - chromium's contributes +3.06 eV/atom, with
#  the binding arriving from the FOURTH shell at 4.8 A - has found a solution
#  the elastic constants permit and the physics does not.  Removing an atom
#  from such a structure removes more repulsion than attraction, which is why
#  the vacancy energy goes negative.
#
#  This constraint was implicit in the hard cutoff, whose restrictiveness kept
#  molybdenum and tungsten out of that region: both were healthy truncated
#  (ratio 0.08-0.10, first shell attractive) and became pathological once the
#  taper opened the reachable region without putting anything physical in the
#  place of what it removed.
...
KB = 8.617333262e-5
#  0.15 and not 0.10: sodium's first shell repels by 0.133 against a cohesive
#  energy of 1.113, which is 0.12 of it, and sodium is sound - ratio 0.00,
#  survives dynamics, positive vacancy energy.  0.10 rejected it.  The three
#  elements this is aimed at are nowhere near the line: chromium 3.06 against a
#  0.61 limit, molybdenum 3.93 against 1.02, tungsten 4.44 against 1.33.
...
def make(el, e, m, D, alpha, r0, gamma, C, s3):
    ...
    pot = L.Potential(m, D, alpha, r0, gamma, C=C, alpha3=s3 * alpha,
                      rcut2=RCUT2_OVER_A * e["a0"], rcut3=rc3, taper=TAPER)
    return cry, pot, dnn
...
def solve(el, e, m, gamma, C, s3, seed=None):
    """
    ...
    def resid(x):
        ...
        if al <= 0.02 or r0 <= 0.3 or not (M_MIN <= mm <= M_MAX):
            return None
        try:
            o = constraints(el, e, (mm, 1.0, al, r0, gamma, C, s3))
...
def evaluate(el, e, m, gamma, C, s3, seed=None):
    x = solve(el, e, m, gamma, C, s3, seed)
    if x is None:
        return None
    hexa = e["struct"] == "hcp"
    al, r0 = x[0], x[1]
    mm = x[2] if hexa else m
    cn = constraints(el, e, (mm, 1.0, al, r0, gamma, C, s3))
    D = e["Ecoh"] / (-cn["E"])
    o = observables(el, e, (mm, 1.0, al, r0, gamma, C, s3))
    dnn = o["dnn"]
    ...
    if REQUIRE_ATTRACTIVE_FIRST_SHELL:
        #  magnitude, not sign.  Iron's first shell is repulsive by 0.06
        #  eV/atom against a cohesive energy of 4.28 and iron is fine - it
        #  survives dynamics and its vacancy energy is positive.  Chromium's
        #  is +3.06 against 4.10.  A sign test rejects both; what separates
        #  them is how much of the cohesive energy the first shell fights.
    ...
    return dict(m=mm, gamma=gamma, D=D, alpha=al, r0=r0, C=C, s3=s3,
                ...
                #  This was hard-coded to 1.12 when RCUT3_OVER_DNN was
                #  introduced, and the consequence was silent: merge_fits
                #  rebuilds each candidate from the stored rcut3 to re-measure
                #  stability, so every fit made at 1.50 was being screened at
                #  1.12 - a different potential - and the good niobium,
                #  tantalum, vanadium and tungsten fits were rejected in favour
                #  of the old ones they beat by an order of magnitude.
                rcut3=dnn*RCUT3_OVER_DNN,
                #  travels with the fit for the same reason rcut3 does: a
                #  record rebuilt without it is a different potential
                taper=TAPER,
                Cij=cij, B=cn["B"]*D, Ecoh=-e["Ecoh"],
                P=cn["P"]*D,
...
#  Both of these take every q at once.  Called one q at a time they rebuild the
#  dynamical matrix from the force-constant dict for each point, which is a few
#  hundred Python iterations per q and turned out to be 91 % of a screened
#  evaluation: 2.2 s against 35 ms for the 8^3 U 9^3 mesh on platinum.
