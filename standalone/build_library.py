...
def main(only=None):
    ...
    for i, (el, p) in enumerate(sorted(fit.items()), 1):
        ...
        d = dict(struct=e["struct"], a0=e["a0"], c_over_a=e.get("c_over_a"),
                 ...
                 #  Brewer's uncertainty on the cohesive energy.  Carried
                 #  through because Ecoh is imposed EXACTLY at every trial
                 #  point while being known only to 0.2-4.6 %, and a reader
                 #  should be able to see that.
                 Ecoh_unc=refdata.ECOH_UNC.get(el),
                 #  the same for the elastic constants, where III/29a averaged
                 #  more than one measurement
                 Cij_unc=refdata.CIJ_UNC.get(el),
        ...
        try:
            ...
            d["mech"] = None          # singular C: the Born check below says why
        ...
        #  standard (Setyawan-Curtarolo) path - comparable with Materials Project
        std = []
        ...
        for n in range(NT):
            ...
            th.append({"T": T, "zpe": t["zpe"], "F": t["F"],
                       "S": t["S"], "Cv": t["Cv"]})
        d["ld"] = {"std": std, "thermo": th,
                   "maxfreq": float(f.max()*L.CM1)}
        ...
        print(f"  [{i:2d}/{len(fit)}] {el:3s} rms={d['rms']:5.1f}%  "
              ...
              f"S298={th[11]['S']:6.2f}{bad}", flush=True)
